...
class AnimalShelterPredictor:
        ...
        try:
            if file_path.endswith('.csv'):
                try:
                    ...
                    # 실패 시 cp949로 재시도 (한글 Windows 환경)
                    df = pd.read_csv(file_path, encoding='cp949')
            elif file_path.endswith('.json'):
                try:
                    df = pd.read_json(file_path, encoding='utf-8')
                except UnicodeDecodeError:
                    df = pd.read_json(file_path, encoding='cp949')
            else:
    ...
    def train_or_load_model(self):
        ...
        if os.path.exists(self.model_save_path):
            ...
            print("테스트 데이터 레이블 분포:", dict(zip(unique, counts)))
    ...
    def predict_top_n_orgnms_next_week(self, start_date_str="2025-08-01", end_date_str="2025-08-07", top_n=5):
        ...
        return predictions_to_save # 예측 결과 리스트 반환
